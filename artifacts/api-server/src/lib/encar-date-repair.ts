 ...
 *
 * Keep aligned with artifacts/verifykm/src/lib/encar-date-repair.ts
 */
...
/** US / provider calendar labels → YYYY-MM-DD (exported for lot date parsing). */
export function providerCalendarLabelToIso(text: string): string | null {
...
export function sanitizeReportIsoDate(
  ...
  const fromCalendar = providerCalendarLabelToIso(trimmed);
  if (fromCalendar) {
